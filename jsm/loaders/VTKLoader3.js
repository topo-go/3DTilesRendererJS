...
class VTKLoader3 extends Loader {
		...
		function parseXML( stringFile ) {
			...
			function xmlToJson( xml ) {
				...
				if ( xml.hasChildNodes() ) {
					...
					if (obj._childList.length <= 1) delete obj._childList
			...
			function readerHeader(uint8, headerType) {
				...
				if (headerType === 'UInt64') {
					...
					let _uint = new Uint32Array(uint8.buffer, 0, 6);

					const _nbBlocks = _uint[0];
					const _s = _uint[2];
					const _s2 = _uint[4];
					const _resultArray = [_offset, _nbBlocks, _s, _s2];
					_uint = new Uint32Array(uint8.buffer, 3 * 8, _nbBlocks * 2);

					for (let i = 0; i < _nbBlocks; i++) {
			...
			function uncompressBlock(compressedUint8, output) {
				const uncompressedBlock = fflate.decompressSync(compressedUint8);
			...
			function parseAppendedData( ele ) {

				const piece = json.PolyData.Piece;
				let appendedData = ele.AppendedData;
				let compressor = ele.attributes.compressor

				let headerType = ele.attributes['header_type']
				let byteOrder = ele.attributes['byte_order']

				let content;

				const encoding = appendedData.attributes.encoding
				if (encoding === 'base64') {
					// substr(1) is to remove the '_' prefix
					content = appendedData['#text'].trim().substring(1);
				}
				if (!piece._childList) {
					return
				}
				...
				for (let i = 0; i < piece._childList.length; i++) {
					let nodeName = piece._childList[i]
					let node = piece[nodeName]
					if (node.DataArray) {
						if (Array.isArray(node.DataArray)) {
							arrayElems.push(...node.DataArray)
						} else {
							arrayElems.push(node.DataArray)
						}
					}
				...
				for (let i = 0; i < arrayElems.length; i++) {
					let offset = Number(arrayElems[i].attributes.offset)
					let nextOffset = 0
					if (i === arrayElems.length - 1) {
						nextOffset = content.length || content.byteLength
					} else {
						nextOffset = Number(arrayElems[i + 1].attributes.offset)
					}

					if (encoding === 'base64') {
						dataArrays.push(Base64toByteArray(content.substring(offset, nextOffset)))
						// arrayElems[i].text = Base64toByteArray(content.substring(offset, nextOffset))
						newArrayElems.push(arrayElems[i])
					} else {
				...
				if (compressor === 'vtkZLibDataCompressor') {
					for (let arrayidx = 0; arrayidx < dataArrays.length; ++arrayidx) {
						...
						let compressedOffset = dataArray.length - (header.reduce(function (a, b) {
							return a + b;
						...
						const uncompressed = new Uint8Array(_buffer);
						const output = {
							offset: 0,
							uint8: uncompressed
						};

						for (let _i2 = 0; _i2 < nbBlocks; _i2++) {
							const blockSize = header[4 + _i2];
						...
						const data = new Uint8Array(uncompressed.length); // set length header
				...
				for (let i = 0; i < newArrayElems.length; i++) {
					newArrayElems[i].text = new TYPED_ARRAY[newArrayElems[i].attributes.type](dataArrays[i].buffer)
				}
			}
			...
			if ( json.PolyData ) {
				...
				if (json.AppendedData) {
					parseAppendedData(json, compressed);
				}
