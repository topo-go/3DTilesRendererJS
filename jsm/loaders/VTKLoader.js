...
class VTKLoader extends Loader {
	...
	parse( data ) {
		...
		function Float64Concat( first, second ) {
			...
			const firstLength = first.length, result = new Float64Array( firstLength + second.length );
		...
		function parseXML( stringFile ) {
			...
			function parseDataArray( ele, compressed ) {
				...
				if ( ele.attributes.format === 'binary' && compressed ) {
					...
					if ( ele.attributes.type === 'Float32' || ele.attributes.type === 'Float64' ) {
					...
					if ( ele.attributes.type === 'Float64' ) {
			...
			if ( json.PolyData ) {
						...
						switch ( sections[ sectionIndex ] ) {
							...
							case 'PointData':

								{
									...
									const propsName = section.attributes.Scalars;
									...
									if ( numberOfPoints > 0 ) {
										...
										for ( let i = 0, len = arr.length; i < len; i ++ ) {
											...
											} else if ( propsName === arr[ i ].attributes.Name ) {

												const components = arr[ i ].attributes.NumberOfComponents || 1
												props = new TYPED_ARRAY[arr[ i ].attributes.type]( numberOfPoints * components );
												props.set( arr[ i ].text, 0 );
				...
				for (let i = 0; i < points.length; i+=3) {
					newPoints.push(points[i], points[i+2], points[i+1])
				}

				const geometry = new BufferGeometry();
				geometry.setIndex( new BufferAttribute( indices, 1 ) );
				geometry.setAttribute( 'position', new BufferAttribute( new Float32Array(newPoints), 3 ) );

				let components = props.length * 3 / points.length
				if (components > 0) {
					geometry.setAttribute( 'property', new BufferAttribute( props, components ) );
				}
